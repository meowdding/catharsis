...
sealed interface PackConfigOption {
    ...
    fun addToDefault(json: JsonObject) {
        ...
        if (json.has(id)) Katharsis.error("Duplicate pack config option id '$id' found when generating default config!")
        json.add(id, value)
    }
    ...
    data class Separator(val title: Component = Component.empty(), val description: Component = Component.empty()) : PackConfigOption {
        override val type: MapCodec<out PackConfigOption> = KatharsisCodecs.getMapCodec<Separator>()
    ...
    data class Information(val title: Component, val description: Component) : PackConfigOption {
        override val type: MapCodec<out PackConfigOption> = KatharsisCodecs.getMapCodec<Information>()
        override val id: String? = null
        override fun title(value: String?): Component = title
        override fun description(value: String?): Component = description
    }
    ...
    data class Bool(
        ...
        override val type: MapCodec<out PackConfigOption> = KatharsisCodecs.getMapCodec<Bool>()
    ...
    data class Dropdown(
        ...
        companion object {
            ...
            val CODEC: MapCodec<out PackConfigOption> = KatharsisCodecs.getMapCodec<Dropdown>().validate {
    ...
    data class Select(
        ...
        companion object {
            ...
            val CODEC: MapCodec<out PackConfigOption> = KatharsisCodecs.getMapCodec<Select>().validate {
    ...
    data class Color(
        ...
        override val type: MapCodec<out PackConfigOption> = KatharsisCodecs.getMapCodec<Color>()
        override val asJson: JsonElement get() = JsonPrimitive(default)
        override fun title(value: String?): Component = title
        override fun description(value: String?): Component = description
    }
    ...
    data class Tab(
        ...
        companion object {
            val CODEC: MapCodec<Tab> = KatharsisCodecs.getMapCodec<Tab>().validate {
                when {
    ...
    companion object {
        ...
        init {
            ID_MAPPER.put("separator", KatharsisCodecs.getMapCodec<Separator>())
            ID_MAPPER.put("information", KatharsisCodecs.getMapCodec<Information>())
            ID_MAPPER.put("boolean", KatharsisCodecs.getMapCodec<Bool>())
            ID_MAPPER.put("dropdown", Dropdown.CODEC)
            ID_MAPPER.put("select", Select.CODEC)
            ID_MAPPER.put("color", KatharsisCodecs.getMapCodec<Color>())
            ID_MAPPER.put("tab", Tab.CODEC)
        }

        @JvmStatic
        fun fromResource(resources: PackResources): List<PackConfigOption>? {
            return Katharsis.runCatching("Loading pack config options from resources") {
                resources.getRootResource("config.katharsis.json")?.get()?.use { stream ->
                    stream.reader().use { reader ->
                        CODEC.codec().listOf().parse(JsonOps.INSTANCE, GsonHelper.parseArray(reader))
                            .ifError { Katharsis.error("Failed to parse config for pack: $it") }
                            .result()
                            .orElse(null)
                    }
                }
