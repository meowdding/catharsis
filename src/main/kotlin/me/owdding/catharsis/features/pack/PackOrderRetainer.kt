...
object PackOrderRetainer {
    ...
    private val savePath = McClient.config.resolve("catharsis/pack_order.json")
    ...
    private fun loadOrder() {
        try {
            ...
            Catharsis.error("Pack order could not be loaded: ${savePath.readText()}", e)
        }
    }
    ...
    fun saveCurrentOrder(packIds: Collection<String>, isServerPack: Predicate<String>) {
        ...
        Scheduling.async {
            try {
                ...
                Catharsis.error("Pack order could not be saved: ${e.message}", e)
            }
        }
    }
