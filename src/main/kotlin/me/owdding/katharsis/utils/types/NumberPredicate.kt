...
sealed interface IntPredicate {
    ...
    companion object {
        ...
        @IncludedCodec
        val CODEC: Codec<IntPredicate> = Codec.either(KatharsisCodecs.getCodec<Range>(), setCodec)
            .xmap(
                { Either.unwrap(it) },
                {
                    when (it) {
...
sealed interface FloatPredicate {
    ...
    companion object {
        ...
        @IncludedCodec
        val CODEC: Codec<FloatPredicate> = Codec.either(KatharsisCodecs.getCodec<Range>(), setCodec)
            .xmap(
                { Either.unwrap(it) },
                {
                    when (it) {
