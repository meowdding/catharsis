...
public abstract class LivingEntityRendererMixin<S extends LivingEntityRenderState, M extends EntityModel<? super S>> {
    ...
    private void katharsis$swapOutModel(S renderState, PoseStack poseStack, SubmitNodeCollector nodeCollector, CameraRenderState cameraRenderState, Operation<Void> original) {
        M originalModel = this.model;

        var customVariant = renderState.katharsis$getCustomEntityModelVariant();
    ...
    private RenderType katharsis$modifyEntityTexture(RenderType renderType, @Local(argsOnly = true) S renderState) {
        var customVariant = renderState.katharsis$getCustomEntityModelVariant();

        if (customVariant != null) {
            return katharsis$createEntityTextureRenderType(renderState, customVariant);
        }
    ...
    private boolean katharsis$modifyEntityLayers(
        ...
        var customVariant = entityRenderState.katharsis$getCustomEntityModelVariant();
        return customVariant == null;
    }

    @Unique
    private RenderType katharsis$createEntityTextureRenderType(S renderState, CustomEntityModel.Variant customVariant) {
