...
import me.owdding.katharsis.features.properties.DataTypeEntry
import me.owdding.katharsis.features.properties.DataTypeProperties
import me.owdding.katharsis.features.properties.NumbericalDataTypeEntry
...
object ItemUtils {
    ...
    private fun RegisterCommandsEvent.registerCommands() {
        registerWithCallback("katharsis dev hand_id") {
            val item = McPlayer.heldItem

            if (item.isEmpty) {
                Text.of("Not holding any item!").sendWithPrefix("katharsis-held-item-id-error")
            } else {
                val id = SkyBlockIdentifierResolver.getCustomLocation(item)

                if (id == null) {
                    Text.of("Item has no custom id!").sendWithPrefix("katharsis-held-item-no-id")
                } else {
                    Text.of("Held item has id ") {
                        ...
                        append("[COPY]") {
                            ...
                            onClick {
                                Text.of("Copied item id to clipboard!", CatppuccinColors.Frappe.yellow).sendWithPrefix("katharsis-held-item-copied-id")
                                McClient.clipboard = id.path
                            }
                        }
                    }.sendWithPrefix("katharsis-held-item-id-$id")
                }
            }
        }

        registerWithCallback("katharsis dev hand_data_types") {
            val item = McPlayer.heldItem
            ...
            if (item.isEmpty) {
                ...
                Text.of("Item has $dataTypeSize data type${"s".takeUnless { dataTypeSize == 1 }.orEmpty()}") {
                    ...
                    onClick {
                        Text.of("Copied item property data to clipboard!").sendWithPrefix("katharsis-hand-data-copy")
                ...
                }.sendWithPrefix("katharsis-hand-data-types")
            }
        }
    }
