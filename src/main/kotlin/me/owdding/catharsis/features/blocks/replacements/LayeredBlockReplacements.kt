...
data class LayeredBlockReplacements(
    ...
    data class Completable(val definitions: List<BlockReplacement.Completable>) {
    ...
    data class LayeredBlockReplacementSelector(val blockReplacementSelectors: List<BlockReplacementSelector>) : BlockReplacementSelector {
        override fun select(state: BlockState, pos: BlockPos, random: RandomSource): BlockReplacementEntry? =
            blockReplacementSelectors.firstNotNullOfOrNull { it.select(state, pos, random) }
    }
