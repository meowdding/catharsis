...
sealed interface GuiMenuType {
    ...
    class Minecraft(val type: MenuType<*>) : GuiMenuType {
        ...
        override fun matches(screen: AbstractContainerScreen<*>): Boolean {
            return (screen.menu as? AbstractContainerMenuHook)?.`katharsis$getMenuTypeOrNull`() == this.type
        }
    }
    ...
    enum class Custom(
        ...
        INVENTORY(Katharsis.id("inventory"), { it is InventoryScreen }),
        GENERIC(Katharsis.id("generic"), { it is ContainerScreen }),
        ;
