...
import me.owdding.katharsis.utils.extensions.toVector3f
    ...
    override fun getChild(name: String): ModelPart {
        return if (super.hasChild(name)) {
            ...
            if (child is SafeModelPart) {
                ...
                Katharsis.warn("model ${this.name} bone $name is not a SafeModelPart!")
                DummyModelPart()
            }
        } else if (missingBones.add(name)) {
            Katharsis.warn("model ${this.name} missing bone $name!")
            DummyModelPart()
        } else {
            DummyModelPart()
        }
    ...
    override fun createPartLookup(): Function<String, ModelPart?> {
        ...
        return Function { boneName: String ->
            ...
            if (parentBone != null) {
                ...
                Katharsis.warn("model ${this.name} missing bone $boneName!")
                DummyModelPart()
            } else {
                DummyModelPart()
            }
