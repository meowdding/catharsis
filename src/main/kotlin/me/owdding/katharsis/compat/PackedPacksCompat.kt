...
object PackedPacksCompat : PackedPacksInitializer {
    ...
    val canEditConfigSprites = WidgetSprites(Katharsis.id("cog"), Katharsis.id("cog_highlighted"))
    val cantEditConfigSprites = WidgetSprites(Katharsis.id("cog_error"), Katharsis.id("cog_error"))
    val warningIncompatSprites = WidgetSprites(Katharsis.id("incompat_warning"), Katharsis.id("incompat_warning"))

    override fun onInitialize(api: PackedPacksApi) {
        val id = Katharsis.id("config_button")
        ...
        api.eventBus().register(InitializePackEntryEvent::class.java, id) { event ->
            ...
            val config = event.packContext().pack().`katharsis$getConfig`()
            val meta = event.packContext().pack().`katharsis$getMetadata`()

            if (config.isNullOrEmpty() && meta == null) return@register

            val requiresLoaded = event.packContext().pack().`katharsis$requiresPackToOpenConfig`()
