...
@Module
object GuiDefinitions : SimplePreparableReloadListener<Map<Identifier, GuiDefinition>>(), KatharsisLogger by Katharsis.featureLogger() {

    val uiDefinitionConverter: FileToIdConverter = FileToIdConverter.json("katharsis/guis")
    ...
    override fun prepare(
        ...
        return uiDefinitionConverter.listMatchingResources(resourceManager).mapBothNotNull { (key, value) ->
            if (key.namespace == Katharsis.MOD_ID) {
    ...
    private fun FinishRepoLoadEvent.finish() {
        KatharsisRemoteRepo.listFilesInDirectory("guis").forEach { (name, path) ->
            runCatching("Error loading remote gui definition $name") {
                ...
                parsed.resultOrPartial().ifPresent { definition ->
                    repoDefinitions.add(DefinitionEntry(Katharsis.id(name.removeSuffix(".json")), definition))
                }
            }
        }
        sortDefinitions()
    ...
    init {
        Katharsis.registerClientReloadListener(Katharsis.id("gui_definitions"), this)
    }
