...
import me.owdding.katharsis.features.armor.models.hook
...
import me.owdding.katharsis.generated.EnumCodec
import me.owdding.katharsis.utils.extensions.isEnum
import me.owdding.katharsis.utils.extensions.isNumber
import me.owdding.katharsis.utils.extensions.set
...
object DataTypeProperties {
    ...
    init {
        ...
        dataTypes.filterType<Collection<*>>().forEach {
            register(it, KatharsisCodecs.getCodec()) { collection -> collection.size }
        }
    ...
    @JvmName("registerInt")
    private fun register(type: DataType<Int>) = register(type, KatharsisCodecs.getCodec())

    @JvmName("registerFloat")
    private fun register(type: DataType<Float>) = register(type, KatharsisCodecs.getCodec())

    @JvmName("registerDouble")
    private fun register(type: DataType<Double>) = register(type, KatharsisCodecs.getCodec())

    @JvmName("registerLong")
    private fun register(type: DataType<Long>) = register(type, KatharsisCodecs.getCodec())

    @JvmName("registerShort")
    private fun register(type: DataType<Short>) = register(type, KatharsisCodecs.getCodec())
    ...
    data class SelectDataTypeItemProperty<Type, CompareType : Any>(val entry: DataTypeEntry<Type, CompareType>) : SelectItemModelProperty<CompareType> {
        ...
        companion object {
            ...
            private fun <Type, CompareType : Any> createType(): SelectItemModelProperty.Type<SelectDataTypeItemProperty<Type, CompareType>, CompareType> {
                ...
                type.hook.`katharsis$setArmorSwitchCodec`(
                    stringTypes.codec(Codec.STRING).dispatchMap(
                        "data_type",
                        { case -> (case.property as SelectDataTypeItemProperty).entry },
                ...
                type.hook.`katharsis$setTooltipSwitchCodec`(
                    stringTypes.codec(Codec.STRING).dispatchMap(
                        "data_type",
                        { case -> (case.property as SelectDataTypeItemProperty).entry },
