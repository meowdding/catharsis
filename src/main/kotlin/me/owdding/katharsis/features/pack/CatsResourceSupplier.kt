...
class CatsResourceSupplier(path: Path) : Pack.ResourcesSupplier {
    private val file = Katharsis.runCatching("Failed to open cats pack at $path") {
            ...
            for ((name, entry) in entry.entries()) {
                if (entry is CatsEntry.Directory) {
                    ...
                    if (id != null) {
                        ...
                        Katharsis.warn("Invalid path in .cats pack $namespace:${name.removePrefix(prefix)}")
                    }
                }
            }
        }
        ...
        for ((name, entry) in rootEntry.entries()) {
            if (entry is CatsEntry.Directory) {
                ...
                if (Identifier.isValidNamespace(namespace)) {
                    ...
                    Katharsis.warn("Non [a-z0-9_.-] character in namespace $namespace in .cats pack")
                }
            }
        }
        return namespaces
