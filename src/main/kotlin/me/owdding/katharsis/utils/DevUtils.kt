...
internal fun debugToggle(path: String, description: String = path): DebugToggle {
    return DebugToggle(Katharsis.id(path), description, KatharsisDevUtils)
}
...
internal fun <T : Any> debugSelect(
    ...
    return DebugSelect(Katharsis.id(path), description, KatharsisDevUtils, initialState, toString, states)
}

@Module
internal object KatharsisDevUtils : DevUtils() {

    private val path = System.getProperty("katharsis.property_path")?.let { Path(it) } ?: McClient.config.resolve("katharsis.properties")
    override val commandName: String = "katharsis dev toggle"
