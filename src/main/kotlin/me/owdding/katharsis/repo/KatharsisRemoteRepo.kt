package me.owdding.katharsis.repo
...
private const val REMOTE_URL = "katharsis-repo.pages.dev"

object KatharsisRemoteRepo : KatharsisLogger by Katharsis.featureLogger() {

    private val gson: Gson = GsonBuilder().create()
    val cacheDirectory: Path= McClient.config.resolveSibling("katharsis-repo-cache")
    ...
    fun initialize(version: String, callback: () -> Unit) {
        ...
        KatharsisRemoteRepo.version = version.takeUnless { it == "main" }
    ...
    fun getFileContent(file: String) = cacheDirectory.resolve(file).takeIf { it.exists() }?.readText(Charsets.UTF_8) ?: run {
        Katharsis.warn("Requested unknown file $file from remote repo! Version=$version, Force Backup=$forceBackupRepo")
        null
    }
    ...
    private fun HttpClient.downloadOrUpdate(remoteHash: String, oldFiles: MutableMap<String, Path>) {
        ...
        val remoteIndex = getJsonObject("index.json") ?: run {
            Katharsis.warn("Failed to load repo data, falling back to backup repo!")
            loadBackupRepo()
            return
        }
        ...
        cache.forEach { (key, value) ->
            ...
            if (!key.startsWith(cacheDirectory)) {
                Katharsis.warn("Bad key found! Skipping $key!")
                return@forEach
            }
    ...
    private fun getFromBackup(path: String): ByteArray? = KatharsisRemoteRepo::class.java.getResourceAsStream("/repo/$path")?.readAllBytes()
